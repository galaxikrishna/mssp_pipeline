...
function polcal_mssp_prep(loc,slop,x,y,o,dat_siz)

    
    s=strcat('"C:\Program Files\Exelis\IDL84\bin\bin.x86_64\idl.exe"',{' '},'"E:\optics and project related stuff\mssp_data_analysis_gui\mssp_codes\call_idl_polcal.pro" -arg',{' '},loc);
    dos([s{:}]);
    
    
    ft=fittype('a*exp(-(x-b)^2/(2*c^2))+d');
    xxx=x;
    curv=fitsread(strcat(loc,'line_pos_polcal.fits'));
    curv(isinf(curv))=0;
    curv(isnan(curv))=0;
