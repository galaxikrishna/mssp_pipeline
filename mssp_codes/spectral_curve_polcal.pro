pro spectral_curve_polcal,dat_path ;the program should be in the folder having data ans line_pos.txt file. Inputs are current path
  cd,dat_path
  readcol,'line_pos.txt',a,b
  a=floor(a)-1
  b=floor(b)-1

  spawn,'dir /b polcal_*',dat
  ...
  for ct=0,n_elements(dat)-1  do begin
    ...
    print,'done '+num2str(ct)+' polcal-files'
  endfor

  writefits,'line_pos_polcal.fits',curv
  exit
  end
